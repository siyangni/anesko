...
# Test Shiny App with ElephantSQL
# This script configures and tests your app with the cloud database

echo "🧪 Testing Shiny App with ElephantSQL"
echo "===================================="
echo ""

# Check if .env file exists
if [ ! -f "shiny-app/.env" ]; then
    echo "❌ ElephantSQL configuration not found. Please run ./setup_elephantsql.sh first"
    exit 1
fi
...
POOL_SIZE_MIN <- 1
POOL_SIZE_MAX <- 3  # Reduced for ElephantSQL free tier
...
# Test Shiny App Components with ElephantSQL
setwd("shiny-app")
...
cat("\nIf all tests passed, your app should work with ElephantSQL.\n")
EOF
...
if [ "$start_app" = "y" ]; then
    echo "🎬 Starting Shiny app with ElephantSQL..."
    echo ""
